...
client.on("messageReactionAdd", async (reaction, user) => {
    ...
    if(reaction.emoji.name == "✅" && reaction.message.id == '851879702695247892') {
        reaction.message.channel.send('Hey It seems like you are going On Duty, Now whenever a ticket gets opened you will be pinged.').then(m => client.setTimeout(() => { if(!m.deleted) m.delete() }, 5000))
...
client.on("messageReactionRemove", async (reaction, user) => {
    ...
    if(reaction.emoji.name == "✅" && reaction.message.id == '851879702695247892') {
        ...
        reaction.message.channel.send('Hey, It seems like you aren\'t on Duty anymore, Have a good rest of your day').then(m => client.setTimeout(() => { if(!m.deleted) m.delete() }, 5000))
    }
})